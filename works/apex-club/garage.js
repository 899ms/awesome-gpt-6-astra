...
const car=createArmoredKart();car.getObjectByName('Race driver').visible=false;scene.add(car);
...
document.querySelector('#driver').addEventListener('change',e=>car.getObjectByName('Race driver').visible=e.target.checked);document.querySelector('#wireframe').addEventListener('change',e=>car.traverse(o=>{if(o.isMesh&&o.material)o.material.wireframe=e.target.checked;}));
document.querySelectorAll('[data-color]').forEach(b=>b.addEventListener('click',()=>{car.traverse(o=>{if(o.material?.userData.bodyPaint)o.material.color.setHex(parseInt(b.dataset.color,16));});document.querySelectorAll('[data-color]').forEach(x=>x.classList.toggle('active',x===b));}));
